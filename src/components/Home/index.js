...
class Home extends Component {
  state = {
    initialMonth: initialMonthsList[0],
    date: '01',
    initialActiveEmoji: emojisList[0].id,
    day: 'Sun',
    initialEmojiName: 'Very Happy',
    initialimageUrl: '',
  }

  changeDate = (date,id) => {
    this.setState({date: date})
    for (let i of initialMonthsList) {
      const {initialActiveEmoji} = this.state
      if (i.dates.map(each => each.emojiUrl === '')) {
        for (let j of emojisList) {
          if (initialActiveEmoji === j.id) {
           this.setState({initialimageUrl:j.emojiUrl})
  ...
  onChangeEmojiName = e => {
    this.setState({initialEmojiName: e.target.value})
  }
  onChangeDay = e => {
  ...
  onClickLeft = () => {
    ...
    const currentIndex = initialMonthsList.indexOf(initialMonth)
    const prevIndex =
      (currentIndex - 1 + initialMonthsList.length) % initialMonthsList.length
    this.setState({initialMonth: initialMonthsList[prevIndex]})
  }

  onClickRight = () => {
    const {initialMonth} = this.state
    const currentIndex = initialMonthsList.indexOf(initialMonth)
    const nextIndex = (currentIndex + 1) % initialMonthsList.length
    this.setState({initialMonth: initialMonthsList[nextIndex]})
  }

  changetoActive = id => {
    this.setState({initialActiveEmoji: id})
  }
  ...
  render() {
    ...
    const {month, monthName} = initialMonth
        ...
        <div className="container">
          <div className="calender">
            <div className="icons-card">
              ...
              <h1 className="calender-month-heading">{monthName}</h1>
              <button
            ...
            <ul className="day-card">
              {daysList.map(eachDay => (
                <li key={eachDay.id} className="day-day">
                  {eachDay.day}
                </li>
              ))}
            </ul>
            <ul className="dates-container">
              {initialMonth.dates.map(eachMonth => (
                <Calender
                  details={eachMonth}
          ...
          <div className="emoji-day-container">
            <ul className="emojis-card">
              {emojisList.map(eachEmoji => (
                <EmojiItem
                  ...
                  details={eachEmoji}
                  initialActiveEmoji={initialActiveEmoji}
                  isActive={this.state.initialActiveEmoji === eachEmoji.id}
                />
              ))}
            </ul>

            <div className="dates-card">
              <div className="inner-dates-card">
                <select
                  value={initialEmojiName}
                  onChange={this.onChangeEmojiName}
                  className="select-emojiName "
                >
                  {emojisList.map(eachEmoji => (
                    <option
                      className="dropdownOption"
                      value={eachEmoji.emojiName}
                      key={eachEmoji.id}
                    >
                      {eachEmoji.emojiName}
                    </option>
              ...
              <h1 className="home-date-heading">{date}</h1>
            </div>
          </div>
        </div>
      </div>
